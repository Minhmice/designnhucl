...
function json(value: unknown, max: number, name: string): unknown {
  ...
  if (encoded === undefined || encoded.length > max || !isJson(value, new Set())) throw new Error(`Invalid ${name} JSON projection`);
  return value;
}
...
export class AgentRunStore {
  ...
  async transition(ownerOrganizationId: string, runId: string, to: AgentState, options: TransitionOptions | string = {}): Promise<AgentRun> {
    ...
    return this.runner.withTenant(ownerOrganizationId, async tx => { const current = await tx.query<Record<string, unknown>>('SELECT state, lease_owner, lease_expires_at FROM agent_runs WHERE owner_organization_id = $1 AND id = $2 FOR UPDATE', [ownerOrganizationId, runId]); if (!current.rows.length) throw new Error('Run not found for owner'); const currentState = String(current.rows[0]!.state); ensureState(currentState); if (!TRANSITIONS[currentState].includes(to)) throw new Error(`Invalid transition ${currentState} -> ${to}`); if (opts.expectedFrom && opts.expectedFrom !== currentState) throw new Error('Run state mismatch'); const values: unknown[] = [ownerOrganizationId, runId, to, currentState]; let where = 'owner_organization_id = $1 AND id = $2 AND state = $4'; if (opts.leaseOwner) { values.push(opts.leaseOwner); where += ` AND lease_owner = $${values.length}`; } const r = await tx.query<Record<string, unknown>>(`UPDATE agent_runs SET state = $3, updated_at = now() WHERE ${where} RETURNING *`, values); if (!r.rows.length) throw new Error('Run state or lease mismatch'); const run = mapRun(r.rows[0]!); await this.events.appendInTransaction(tx, { ownerOrganizationId, aggregateType: 'agent_run', aggregateId: run.id, eventType: `agent_run.${to}`, payload: { state: to }, ...(opts.actor ? { actor: opts.actor } : {}) }); return run; });
  }
  ...
  async appendStep(ownerOrganizationId: string, runId: string, leaseOwner: string, input: StepInput = {}, now = new Date()): Promise<RunStep> {
    ...
    return this.runner.withTenant(ownerOrganizationId, async tx => { const r = await tx.query<Record<string, unknown>>(`INSERT INTO run_steps (owner_organization_id, run_id, sequence, attempt, state, checkpoint, input_projection, output_projection, error_provenance, lease_owner, lease_expires_at) SELECT $1, $2, COALESCE(MAX(sequence),0)+1, $3, $4, $5::jsonb, $6::jsonb, $7::jsonb, $8::jsonb, a.lease_owner, a.lease_expires_at FROM agent_runs a LEFT JOIN run_steps s ON s.run_id = a.id WHERE a.owner_organization_id = $1 AND a.id = $2 AND a.lease_owner = $9 AND a.lease_expires_at > $10 AND a.state IN ('leased','running','waiting_approval') GROUP BY a.id RETURNING *`, [ownerOrganizationId, runId, input.attempt ?? 1, input.state ?? 'queued', input.checkpoint, input.inputProjection, input.outputProjection, input.errorProvenance, leaseOwner, now]); if (!r.rows.length) throw new Error('Run lease mismatch or expired'); const step = mapStep(r.rows[0]!); await this.events.appendInTransaction(tx, { ownerOrganizationId, aggregateType: 'agent_run', aggregateId: runId, eventType: 'agent_run.step_appended', payload: { sequence: step.sequence, attempt: step.attempt, state: step.state } }); return step; });
  }
