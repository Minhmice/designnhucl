...
SET normalized_legal_name = regexp_replace(lower(trim(legal_name)), '\\s+', ' ', 'g'),
    normalized_jurisdiction = CASE WHEN jurisdiction IS NULL THEN NULL ELSE regexp_replace(lower(trim(jurisdiction)), '\\s+', ' ', 'g') END
